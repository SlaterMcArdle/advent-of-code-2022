...
// Create a placeholder array for the processed horizontal stacks
let finalStacks = [];
...
// Loop through each step and move the crates
for (let i = 0; i < steps.length; i++) {
    // Split the steps apart and grab the amount and column numbers
    let stepArray = steps[i].split(' ');
    const amount = parseInt(stepArray[1]);
    const initialStack = parseInt(stepArray[3]) - 1;
    const finalStack = parseInt(stepArray[5]) - 1;
    // Move the top crate from the starting to ending column the right amount of times
    for (let j = 0; j < amount; j++) {
        let poppedValue = transformedStacks[initialStack].pop();
        console.log(poppedValue);
        transformedStacks[finalStack].push(poppedValue);
    }
}

// Grab the values of each of the top crates
let topCrates = '';
